...
export = class CommandInhibitor extends Inhibitor.Inhibitor {
    ...
    async run(context: CommandContext) {
        ...
        try {
            ...
            if (member.roles.cache.hasAny(...guild.config.commandRoles)) return true;
            if (member.permissions.has('ADMINISTRATOR')) return true;
            return context.safeReply({ ephemeral: true, content: 'You must be a command member to use this command.' });
        } catch {
