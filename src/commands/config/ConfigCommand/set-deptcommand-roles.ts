...
export default {
    name: 'set-deptcommand-roles',
    description: 'Set roles that have access to use powerful commands.',
    arguments: [
        new Argument({
            name: 'roles',
            description: 'The roles that have access to use the commands',
    ...
    run: async (context: CommandContext) => {
        ...
        GetGuildData(context.client, context.guild!.id).then(async data => {
            const oldRoles = data.config.departmentCommandRoles;
            data.config.departmentCommandRoles = roles;
            await SaveGuildData(context.client, context.guild!.id, data).catch(error);
            const successEmbed = SuccessEmbed('Successfully updated `department command` roles.')
