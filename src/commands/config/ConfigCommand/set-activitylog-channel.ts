...
export default {
    name: 'set-activitylog-channel',
    description: 'Set the department activity log channel to be used.',
    arguments: [
        new Argument({
            name: 'channel',
    ...
    run: async (context: CommandContext) => {
        ...
        return GetGuildData(context.client, context.guild!.id).then(async data => {
            data.config.activityLogChannel = channel ? channel.id : Constants.BlankGuild.config.activityLogChannel;
            await SaveGuildData(context.client, context.guild!.id, data).catch(error);
            return context.editReply({ embeds: [ SuccessEmbed('Successfully updated activity log channel.') ] });
        }).catch(error);
    }
}