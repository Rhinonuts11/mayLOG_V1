...
export default {
    name: 'set-loa-channel',
    description: 'Set the department leave of absence request channel to be used.',
    arguments: [
        new Argument({
            name: 'channel',
    ...
    run: async (context: CommandContext) => {
        ...
        return GetGuildData(context.client, context.guild!.id).then(async data => {
            data.config.loaChannel = channel ? channel.id : Constants.BlankGuild.config.loaChannel;
            await SaveGuildData(context.client, context.guild!.id, data).catch(error);
            return context.editReply({ embeds: [ SuccessEmbed('Successfully updated leave of absence channel.') ] });
        }).catch(error);
    }
}