...
export default {
    ...
    description: 'Set the department log channel to be used.',
    arguments: [
        new Argument({
            name: 'channel',
    ...
    run: async (context: CommandContext) => {
        ...
        return GetGuildData(context.client, context.guild!.id).then(async data => {
            data.config.logChannel = channel ? channel.id : Constants.BlankGuild.config.logChannel;
            await SaveGuildData(context.client, context.guild!.id, data).catch(error);
            return context.editReply({ embeds: [ SuccessEmbed('Successfully updated department log channel.') ] });
        }).catch(error);
    }
}