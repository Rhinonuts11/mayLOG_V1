...
const mongo = new MongoClient(process.env.MONGO_URI as string, {
    tlsCAFile: 'certificates/mongodb-maylog.crt'
});
...
async function dbConnect(): Promise<void> {
    try {
        ...
        redis.on('error', error => {
            if (typeof error === 'string' && error.search('ECONNRESET') !== -1) return;
                ...
                try {
                    await client.users.fetch(this.guild.ownerId).then(user => {
                        this.embed.addFields({
                            ...
                            value: `${user.username}${user.discriminator === '0' ? '' : `#${user.discriminator}`} / \`${user.id}\``
                        });
                    }).catch(() => {
                        this.embed.addFields({
                            name: 'Guild Owner Discord Account',
